...
作为论文，目前还不够。更准确的判断是：可以发展成 workshop / course-paper / short-paper 级别的研究原型；如果要投正式交通规划或数据挖掘论文，需要补外部验证、前瞻式事件上下文、统计不确定性和更强的 mode-choice 实验。
...
These numbers are strong enough for a course report because the baseline failure and correction mechanism are visible. They are not enough for a paper if framed as pure prediction SOTA, because exact household-level prediction remains hard and the mode-composition extension is still exploratory.
...
2. External event features: policy stringency, mobility indices, transit recovery, remote-work statistics.
...
5. Uncertainty: bootstrap confidence intervals for weighted MAE, bias, and subgroup gains.
