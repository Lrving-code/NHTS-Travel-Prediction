...
- Transit-share weighted MAE improves from `0.0325` to `0.0269`, but full mode composition improves only modestly. The added trip-level survey-weighted harmonized mode-choice transfer baseline reaches 2022 survey-weighted accuracy `0.9297`, weighted balanced accuracy `0.4343`, and weighted macro F1 `0.4663`, showing a stronger mode-choice check while preserving a rare-mode limitation.
