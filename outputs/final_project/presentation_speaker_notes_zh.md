...
另外我们补了 trip-level survey-weighted harmonized mode-choice transfer baseline：2017 训练的 XGBoost 在 2022 `MODE_GROUP` 上 weighted accuracy `0.9297`、weighted balanced accuracy `0.4343`、weighted macro F1 `0.4663`。这个可以证明方式选择链路更完整，但 rare modes 仍然难，所以不能夸大成完整 mode-choice 已解决。
