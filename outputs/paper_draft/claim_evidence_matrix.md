...
| Historical supervised and count-model baselines struggle on 2022 household trips. | `outputs/final_project/final_metrics_summary.csv`; `outputs/strong_baselines/strong_tabular_baseline_metrics.csv`; `outputs/count_model_baselines/count_model_baseline_metrics.csv`; `outputs/negative_binomial_baseline/negative_binomial_2022_metrics.csv`; `outputs/zero_inflated_count_baseline/zero_inflated_count_metrics.csv`; `outputs/zero_inflated_count_baseline/zero_inflated_count_diagnostics.csv` | "Ordinary XGBoost has weighted bias `+3.6052`; CatBoost GPU has weighted bias `+3.4873`; Poisson GLM has weighted bias `+3.6178`; zero-inflated Poisson has weighted bias `+3.4233`; the negative-binomial GLM has weighted MAE `6.1229` with solver diagnostics recorded." | "Traditional methods cannot model travel demand." |
...
| The LLM design is efficient and auditable. | `outputs/final_project/llm_distillation_deployment_notes.md`; `outputs/llm_event_features/`; `outputs/final_project/0611_final_presentation.pptx` | "7,893 households are aggregated to 1,327 cohorts and processed in 89 batch prompts; inference uses a deterministic adapter." | "The system asks the LLM for every household prediction." |
| Local/open-source LLM replication is implemented as a small-sample GPU control. | `src/run_local_llm_prior_replication.py`; `outputs/local_llm_prior_replication/local_llm_environment_audit.md`; `outputs/local_llm_prior_replication/local_llm_prior_replication_report.md`; `outputs/local_llm_prior_replication/local_llm_replication_interpretation.md`; `outputs/local_llm_prior_replication/local_vs_reference_prior_comparison.csv` | "A CUDA-enabled local Qwen prior run generated valid structured priors for 32 cohorts; agreement with the GPT-reference prior is limited, so this is a reproducibility and sensitivity control rather than the main prior source." | "The open-source local model reproduces the main GPT prior quality at scale." |
...
| Zero-inflated Poisson weighted MAE | `4.1742` | `outputs/zero_inflated_count_baseline/zero_inflated_count_metrics.csv` |
| Zero-inflated Poisson weighted bias | `+3.4233` | `outputs/zero_inflated_count_baseline/zero_inflated_count_metrics.csv` |
...
- Poisson/Tweedie/negative-binomial/zero-inflated count models are transparent reviewer-facing baselines with solver diagnostics recorded; do not present them as fully optimized count-model state of the art.
- Local/open-source LLM replication is a completed 32-cohort GPU sensitivity control; do not present it as a full-scale replacement for the main GPT-reference priors.
