...
| Local/open-source LLM replication is implemented as a small-sample GPU control. | `src/run_local_llm_prior_replication.py`; `outputs/local_llm_prior_replication/local_llm_environment_audit.md`; `outputs/local_llm_prior_replication/local_llm_prior_replication_report.md`; `outputs/local_llm_prior_replication/local_vs_reference_prior_comparison.csv` | "A CUDA-enabled local Qwen prior run generated valid structured priors for four cohorts; agreement with the GPT-reference prior is limited, so this is a reproducibility and sensitivity control rather than the main prior source." | "The open-source local model reproduces the main GPT prior quality at scale." |
...
- Local/open-source LLM replication is a completed four-cohort GPU smoke test; do not present it as a full-scale replacement for the main GPT-reference priors.
