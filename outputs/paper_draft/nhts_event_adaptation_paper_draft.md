...
4. We add robustness and credibility checks: leakage audit, prospective/frozen event context, placebo events, permutation controls, cohort-prior value analysis, bootstrap confidence intervals, uncertainty-aware Pareto selection, equity-aware subgroup analysis, and external ACS/BTS/PSRC validation.
...
- transparent Poisson/Tweedie/negative-binomial/zero-inflated count models;
...
| Zero-inflated Poisson | 4.1742 | 5.2719 | +3.4233 | -0.6091 |
...
The ordinary XGBoost baseline overpredicts strongly. Transparent Poisson/Tweedie and zero-inflated Poisson count-model baselines also overpredict by more than `+3.4` weighted trips, while the negative-binomial GLM reduces positive bias only by becoming unstable and much less accurate. This suggests that the core failure is not only high-capacity tabular overfitting but a broader historical-transfer mismatch under the 2022 event shift. The primary gated adapter reduces weighted MAE by `42.31%` relative to ordinary XGBoost, by `40.05%` relative to the zero-inflated Poisson baseline, by `42.30%` relative to the best transparent GLM count model, and by `59.13%` relative to the negative-binomial GLM; it also reduces weighted RMSE by `32.22%` and absolute weighted bias by `99.36%`. The LLM-only pressure baseline is directionally useful but less calibrated, supporting the hypothesis that LLM priors need a historical household baseline.
