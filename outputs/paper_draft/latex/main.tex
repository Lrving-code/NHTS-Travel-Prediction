...
Transparent Poisson/Tweedie/negative-binomial count-model baselines show the same target-year transfer difficulty.
...
We evaluate the method against historical, trend, global-event, random-prior, LLM-only, zero-shot rule-tree, small-history rule-calibration, transparent count-model, and stronger non-LLM baselines.
We further add cohort-prior value analysis to separate event-level downscaling from selective cohort refinement.
...
Household travel demand prediction is usually grounded in historical survey data and classical travel-demand modeling, including discrete-choice foundations for travel behavior analysis \citep{mcfadden1974conditional,benakiva1985discretechoice}.
...
\begin{figure}[t]
    \centering
    \includegraphics[width=\textwidth]{method_workflow.png}
    \caption{Framework overview. Historical NHTS waves train a routine household predictor, while the LLM branch converts post-pandemic event context and cohort profiles into structured event priors. The final prediction is produced by a fixed label-free adapter; 2022 labels are used only for final evaluation.}
    \label{fig:workflow}
\end{figure}
...
Third, we evaluate against a broad method spectrum, including historical mean, ordinary XGBoost, trend shift, transparent Poisson/Tweedie/negative-binomial count models, CatBoost GPU, LLM-only pressure, global event prior, random prior, zero-shot LLM rule tree, pseudo-label tree, and LLM rule plus historical calibration.
Fourth, we add leakage audits, placebo controls, cohort-prior value analysis, bootstrap confidence intervals, equity-aware subgroup analysis, mode/purpose extensions, and external mechanism checks with ACS and PSRC data \citep{burrows2024commuting,psrc2024regionaltravelstudy}.
...
\paragraph{Data.}
The main dataset is public-use \nhts{} household data for 2001, 2009, 2017, and 2022, with 357,553 harmonized household records and 28 common household variables \citep{fhwa2022nhts,fhwa2024nhtstrends}.
The target 2022 evaluation set contains 7,893 households.
We use the released household final weights for weighted metrics, following the survey-weighting design documented for the 2022 \nhts{} \citep{ipsos2022nhtsweighting}.
...
The comparison includes historical mean, ordinary historical XGBoost, historical trend shift, transparent Poisson/Tweedie/negative-binomial count models, stronger tabular baselines, LLM-only pressure, global event prior, random pressure control, zero-shot LLM rule tree, pseudo-label tree, LLM rule plus historical calibration, and the primary gated LLM event adapter.
...
\begin{tabular}{lrrrr}
\toprule
Method & \whmae{} & wRMSE & \whbias{} & w$R^2$ \\
\midrule
...
Neg-bin GLM ($\alpha=0.5$) & 6.1229 & 15.3423 & -1.1091 & -12.6282 \\
...
LLM rule + 500-history calib. & 2.7723 & 3.8226 & +0.4627 & 0.1538 \\
...
Transparent Poisson/Tweedie count-model baselines also overpredict by more than +3.6 weighted trips, while the negative-binomial GLM reduces positive bias only by becoming unstable and much less accurate.
This suggests that the core failure is not only high-capacity tabular overfitting but broader historical-transfer mismatch under the 2022 event shift.
The primary gated adapter reduces \whmae{} by 42.31\% relative to ordinary XGBoost, by 42.30\% relative to the best transparent count model, and by 59.13\% relative to the negative-binomial GLM; it also reduces weighted RMSE by 32.22\% and absolute weighted bias by 99.36\%.
...
\begin{figure}[t]
    \centering
    \includegraphics[width=0.92\textwidth]{metric_comparison.png}
    \caption{Main metric comparison. Historical tabular baselines have large positive bias on 2022, while event-aware adapters reduce weighted MAE and move bias closer to zero.}
...
\begin{itemize}
    ...
    \item Poisson GLM reaches \whmae{} 4.3368, Tweedie GLM reaches 4.3761, and a stable negative-binomial GLM check reaches 6.1229 with $\alpha=0.5$ while $\alpha=1.0$ is solver-infeasible during historical validation. These transparent baselines are grounded in standard count-data regression practice \citep{cameron2013countdata}, and solver diagnostics are recorded so they are not overclaimed as exhaustively optimized count-model state of the art.
    \item Cohort-prior value analysis shows that the primary gated adapter is 0.0508 \whmae{} lower than a same-alpha global prior and 0.0201 lower than the reported low-cost global prior; it beats same-alpha global prior in 77.8\% of evaluated subgroup cells while using cohort-specific pressure for only 17.5\% of survey-weighted households.
\end{itemize}

\begin{figure}[t]
    \centering
    \includegraphics[width=0.78\textwidth]{permutation_null_mae.png}
    \caption{Permutation robustness check. The real LLM event-pressure assignment yields lower weighted MAE than the random-permutation null distribution, supporting that the event prior contains cohort-level ordering signal beyond arbitrary downscaling.}
...
\begin{figure}[t]
    \centering
    \includegraphics[width=0.49\textwidth]{trip_pareto_frontier.png}
    \includegraphics[width=0.49\textwidth]{behavior_system_improvement.png}
    \caption{Planning-oriented evaluation. Left: Pareto frontier for trip-generation operating points under accuracy, bias, RMSE, and LLM-request cost. Right: behavior-system improvements across trip generation, mode composition, purpose composition, and mode-specific trip volume.}
...
The cohort-prior value analysis refines the interpretation rather than overturning it.
The final gated adapter uses cohort-specific pressure selectively and provides small but consistent MAE/calibration gains over global controls, especially in lower-income, zero-vehicle, and no-worker subgroup cells.
The contribution should therefore be framed carefully as label-free event adaptation with selective auditable cohort refinement, not as evidence that rich LLM reasoning alone dominates.
...
Fourth, mode and purpose extensions are useful for planning relevance. A harmonized trip-level mode-choice transfer baseline is implemented, but rare-mode macro F1 remains limited; purpose-choice is still exploratory.
...
Sixth, the current count-model baselines include transparent Poisson, Tweedie, and stable negative-binomial GLM checks with solver diagnostics recorded; a submission version should still add zero-inflated or more carefully regularized count variants if time permits.
Seventh, a local/open-source LLM prior-replication protocol and environment audit are implemented, and a 32-cohort CUDA sensitivity control has been run on the RTX 4090. The local run is useful as reproducibility evidence, but it should be presented as a sensitivity control rather than a full replacement for the main prior source.
...
\begin{itemize}
    ...
    \item \texttt{outputs/cohort\_prior\_value\_analysis/}
    \item \texttt{outputs/zero\_shot\_llm\_rule\_tree\_baseline/}
    \item \texttt{outputs/llm\_rule\_small\_data\_calibration/}
    \item \texttt{outputs/count\_model\_baselines/}
    \item \texttt{outputs/negative\_binomial\_baseline/}
    \item \texttt{outputs/local\_llm\_prior\_replication/}
