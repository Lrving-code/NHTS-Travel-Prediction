...
- 500-run permutation null for sensitivity LLM pressure: actual wMAE `2.4820`, random mean `2.6425`, empirical p `0.0020`.
...
| LLM cohort-specific ranking is the dominant reason for improvement. | Overclaim | single-objective sensitivity row improves over global rule only 1.60%; permutation p=0.0020. | Say the dominant signal is event-level downscaling; cohort-specific LLM ranking adds incremental evidence. |
